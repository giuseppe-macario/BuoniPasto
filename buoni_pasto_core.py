...
def elabora_pdf(path_pdf: str) -> str:
    ...
    sys.stdout = buffer
    try:
        # --- controlli di base su file/estensione/dimensione ---
        if not path_pdf.lower().endswith(".pdf"):
            print(f"Errore: il file deve avere estensione .pdf (ricevuto: {path_pdf})")
            return buffer.getvalue()

        if not os.path.isfile(path_pdf):
            print(f"Errore: file non trovato: {path_pdf}")
            return buffer.getvalue()

        size_bytes = os.path.getsize(path_pdf)
        if size_bytes > 100 * 1024:
            print(f"Errore: file troppo grande ({size_bytes} byte). Dimensione massima consentita: 100 KB.")
            return buffer.getvalue()

        # ------ VALIDAZIONE ------
        ok, rep = valida_pdf(path_pdf)
        if not ok:
        ...
        with pdfplumber.open(path_pdf) as pdf:
            for page in pdf.pages:
                ...
                for raw_line in text.splitlines():
                    ...
                    if (causale == "RECUPERO COMPENSATIVO"
                        and ora_ing.hour == 7 and ora_ing.minute == 30):
                        entrata_str = "*07:30"
                    risultati.append((
                        f"{data_str} ({wd_it})",
                        entrata_str,
